...
def agpt_proxy1bllama2() -> FaultTolerantTrainer.Config:
    cfg = ezpz_agpt_proxy1bllama2()
    cfg.dataloader.dataset_path = None
    return cfg


def agpt_300m() -> FaultTolerantTrainer.Config:
    cfg = ezpz_agpt_300m()
    cfg.dataloader.dataset_path = None
    return cfg


def agpt_proxy1b() -> FaultTolerantTrainer.Config:
    cfg = ezpz_agpt_proxy1b()
    cfg.dataloader.dataset_path = None
    return cfg
...
def agpt_1b() -> FaultTolerantTrainer.Config:
    return ezpz_agpt_1b()
...
def ezpz_agpt_proxy1bllama2() -> FaultTolerantTrainer.Config:
    cfg=agpt("proxy1bllama2", local_batch_size=2, activation_checkpoint_mode="none")
    ...
    cfg.hf_assets_path = "/eagle/datascience_collab/wkwiecinski/torchtitan/assets/hf/llama-7b"
    return cfg


 
def ezpz_agpt_proxy1b() -> FaultTolerantTrainer.Config:
    cfg=agpt("proxy1b", local_batch_size=2, activation_checkpoint_mode="none")
    cfg.lr_scheduler=LRSchedulersContainer.Config(
        warmup_steps=200,
        decay_ratio=1.0,
        decay_type="linear",
...
def ezpz_agpt_300m() -> FaultTolerantTrainer.Config:
    return agpt("300m", local_batch_size=2, activation_checkpoint_mode="none")
...
def ezpz_agpt_1b() -> FaultTolerantTrainer.Config:
    return agpt("1b", activation_checkpoint_mode="none")
