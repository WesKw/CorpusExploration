...
import tyro
import sys
sys.path.append('/home/wkwiecinski/CorpusExploration/src/')
...
def _load_local_dataset(dataset_path: str, split: str):
    """Load C4 dataset with default configuration."""
    return load_dataset("json", data_files=dataset_path, name="en", split=split, streaming=True)
...
DATASETS = {
    ...
    "none": DatasetConfig(
        path="",
        loader=partial(_load_local_dataset, split="train"),
        sample_processor=_process_c4_text,
    ),
    "difficulty": DatasetConfig(
        path="",
        loader=partial(_load_local_dataset, split="train"),
        sample_processor=_process_c4_text,
    ),
    "difficulty-reverse": DatasetConfig(
        path="",
        loader=partial(_load_local_dataset, split="train"),
        sample_processor=_process_c4_text,
    ),
